...
#define PERL_ARG_RESPONSE_EXCLUDE (_ARG_DQUOTE | _ARG_WILDCARD)
#define PERL_ARG_RESPONSE         (_ARG_NONZERO | _ARG_RESPONSE)
#else
#include <klibc/startup.h>

#define PERL_ARG_RESPONSE_EXCLUDE (__KLIBC_ARG_DQUOTE | \
                                   __KLIBC_ARG_WILDCARD | __KLIBC_ARG_SHELL)
#define PERL_ARG_RESPONSE         (__KLIBC_ARG_NONZERO | __KLIBC_ARG_RESPONSE)
#endif
...
void _response (int *argcp, char ***argvp)
{
  ...
  for (i = 1; i < old_argc; ++i)
    if (old_argv[i] != NULL
        && !(old_argv[i][-1] & PERL_ARG_RESPONSE_EXCLUDE)
        && old_argv[i][0] == '@')
      break;
  if (i >= old_argc)
    return;                     /* do nothing */
